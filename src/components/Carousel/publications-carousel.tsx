          ...
          <CarouselContent>
            {publications.map((pub, index) => (
              <CarouselItem key={pub.id} className="md:basis-1/2 lg:basis-1/3">
                ...
                >
                  {/* Publication Image */}
                  {pub.languages.es?.image  && (
                    <div className="relative h-64 w-full">
                      <Image
                        src={pub.languages.es?.image}
                        alt={pub.languages.es?.title}
                        fill
                  ...
                  {/* Publication Content */}
                  <div className="p-6">
                    <h3 className="text-xl font-bold mb-2 line-clamp-2">{pub.languages.es?.title}</h3>
                    <div className="text-gray-600 mb-3">{pub.date}</div>
                    <p className="text-gray-700 line-clamp-2">{pub.languages.es?.description}</p>
                    
                    {/* Tags */}
                    {pub.languages?.es?.tags && (
                      <div className="mt-4 flex flex-wrap gap-2">
                        {pub.languages.es.tags.slice(0, 2).map((tag) => (
                          <span
                            key={tag}
